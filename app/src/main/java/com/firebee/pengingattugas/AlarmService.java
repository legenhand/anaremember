...
public class AlarmService extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		...
		if (matcher.matchAction(action)) {
			if (POPULATE.equals(action)) {
				PengingatTugas.dbHelper.populate(Long.parseLong(alarmId), PengingatTugas.db);
			...
			if (CANCEL.equals(action)) {
				...
				PengingatTugas.dbHelper.shred(PengingatTugas.db);
			}			
		}
	}
	...
	private void execute(String action, String... args) {		
		...
		if (alarmMsgId != null) {
			c = PengingatTugas.db.query(AlarmMsg.TABLE_NAME, null, AlarmMsg.COL_ID+" = ?", new String[]{alarmMsgId}, null, null, null);
			
		} else {
			c = AlarmMsg.list(PengingatTugas.db, alarmId, startTime, endTime, status);
		}
